...
Go 服务会先将实时状态写入 Redis。启用 ClickHouse 后，已闭合 K 线会写入 ClickHouse。ClickHouse 写入失败的记录会持久化到 Redis 重试队列，并由后台 worker 重试。

ClickHouse 访问实现位于 `pkg/clickhousemarket`。`market-data/internal/store` 只保留服务级入口和 Redis 重试队列，实际 ClickHouse 表初始化、批量写入和 K 线历史读取都由公共包承担。
