...
- `CalculationWindow` 在 runner 长期窗口和窗口快照递推路径启用基础指标流式状态，连续 `Append` 时复用 SMA、EMA、RSI、ATR、MACD、OBV、VWAP 等中间状态。
...
一次本地基线结果为约 `84ms/op`、`6.7MB/op`、`4830 allocs/op`。这说明窗口构造已不再是唯一瓶颈，`CalculateWindow` 的完整特征计算仍是 realtime CPU 的主要优化方向。
...
最新底层指标快照存储在 Redis，并通过 NATS market snapshot 发布给 `strategy-engine`。指标不再写入 ClickHouse；需要历史指标时，从 ClickHouse K 线按需重算。
