...
market-data/
  ...
  internal/indicatorwindow/  # 指标窗口聚合和语义特征生成
...
指标 runner 读取最近已闭合 K 线，并按交易所、市场、交易对和周期写入最新底层指标快照。
...
在底层指标快照之上，Go `indicatorwindow` 会生成窗口聚合特征。它会把最近一段已闭合指标序列压缩成策略更容易消费的语义字段，例如：
