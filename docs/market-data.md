...
Go `strategy-engine` 启动时会用 Redis 特征 hash 恢复初始态，启动后主要消费该服务发布的 NATS market snapshot 更新内存态。
...
- 动量和震荡类：RSI、MACD、STC（23/50/10/3/3，固定窗口流式实现）、KDJ、Stochastic、Stoch RSI、SKDJ、CCI、Williams %R、ROC。
...
- 闭合指标和窗口快照写 Redis 时使用合并 pipeline；冷启动补齐的同一 symbol/interval 指标历史也按任务批量写入一个 Redis pipeline，避免为最近 `50` 个 snapshot 逐条往返。
...
- `CalculationWindow` 在 runner 长期窗口和窗口快照递推路径启用基础指标流式状态；除 SMA、EMA、RSI、ATR、ADX/DI、MACD、OBV、VWAP、WaveTrend 和 MoneyFlow 外，还连续维护 AlphaTrend、Dynamic Swing VWAP、VFI、Heikin Ashi 及多种 TradingView 指标末值。EMA、SMA、volume SMA 和 MACD 状态按配置动态创建，不依赖固定槽位编号。
- 当前未收盘 K 线 realtime 计算使用缓存窗口 clone 后追加临时 K 线，避免每个 open tick 都从 K 线字符串重建完整计算窗口；preview K 线列表使用 closed base + 单根 overlay 惰性物化，OHLCV 和可变指标历史仍深拷贝隔离。AI Source preview 使用已有 prefix 和 Wilder ATR 单点递推，不再为每个 tick 重算完整 ATR 序列。重复 open time 的替换场景仍保留 rebuild 语义。
...
- `pkg/indicatorcalc.CalculateWindows` 支持在固定 warmup 后连续计算结果后缀，runner 可用它补齐缺失的 recent 指标 snapshot，避免冷启动窗口分析阶段反复回放同一批历史 K 线。
- `CalculateWindow` 正常路径复用已经解析好的 `CalculationWindow` series 做数据质量检查，解析失败时保留原有 fallback 以维持 `invalid_open`、`invalid_high` 等错误原因。
- Nadaraya-Watson envelope 使用栈上权重缓存减少重复权重计算；MoneyFlow 中的区间最高/最低扫描已抽成公共 helper，供 volume profile 和供需区间复用。

最近一次小样本本地指标负载验证使用 `symbols=2`、四个模拟交易所、服务当前周期集合、`lookback=300`、`warmup=250`、`window-lookback=50`、`runs=2` 和 `advance-each-run`。结果中 `tasks=56`、`calculate_window_calls=2856`，等于冷启动 `56 * 50` 个指标点加第二轮稳态 `56 * 1` 个新增收盘指标；`redis_writes=2968` 包含底层指标和窗口快照写入。该结果只验证计算次数收敛，不是生产 SLA。

2026-07-20 的冷启动采样使用四个模拟交易所、每交易所 `100` 个 symbol、`lookback=268`、`warmup=268` 和 `runs=1`。当前版本三次有效样本为 `13.26s`、`18.21s`、`16.28s`，中位数 `16.28s`；Git HEAD 基线为 `21.51s`、`23.44s`、`21.82s`，中位数 `21.82s`。在 2800 个任务的采样规模下，当前版本耗时约降低 25%。但每交易所 `500` 个 symbol、合计 14000 个任务的当前版本实跑超过 `4m38s` 仍未完成，RSS 一度约 `3GB`，显示窗口和 snapshot 常驻量会造成非线性 GC 压力；不得把 100-symbol 吞吐直接线性外推为 top500 SLA。
...
早期本地基线约 `84ms/op`、`6.7MB/op`、`4830 allocs/op`。在固定 warmup、窗口/指标拆分、部分指标流式化、重复解析削减和 AI Source preview 单点递推后，2026-07-12 同机五轮结果约 `0.93-1.27ms/op`、`329.5KB/op`、`426 allocs/op`。本机 benchmark 只用于观察相对趋势，不是生产 SLA；完整 realtime 特征仍会 clone 隔离窗口以避免污染 closed state。
...
在线 Go `strategy-engine` 启动时从 Redis 读取 Go 已聚合的窗口特征恢复初始态，启动后主要消费 NATS market snapshot，不要求实时路径每次从 ClickHouse 拉取几十到几百根 K 线并本地计算指标。需要更长历史时，应读取 ClickHouse K 线并按需计算指标。
...
该接口面向后续历史回填、回放验证和回测服务。当前 Go 在线策略路径启动时可从 Redis 特征 hash 恢复，启动后优先消费 NATS market snapshot，不依赖每次从 ClickHouse 拉取历史。
...
该负载测试不会连接真实交易所、Redis 或 ClickHouse。它使用 realtime collector 路径，并等待模拟 WebSocket 明确进入运行态后才开始发压；模拟行情事件和假的 store 延迟可用于检查队列长度、latest-event 合并/丢弃和 worker 吞吐。

运行指标 runner 负载测试：

```sh
cd backend/go-service/market-data
go run ./cmd/market-data-indicator-loadtest -symbols=500 -lookback=310 -warmup=250 -window-lookback=50 -snapshot-cache-limit=60 -runs=2 -advance-each-run
```

指标负载测试会使用服务当前周期集合和假的 K 线/store 数据模拟四个交易所。它会分别输出 cold/steady 每轮耗时，以及 recent 指标缓存读写、命中/未命中、`calculate_window_calls`、snapshot 写入数和批量 Redis round-trip 数，不会写入真实存储。带 `-advance-each-run` 时，第二轮开始每个任务只追加一根新收盘 K 线，可用于观察稳态是否只补一根底层指标。

评估 top500 冷启动时先使用 `-symbols=100 -lookback=268 -warmup=268 -runs=1` 做至少三次交错采样，再设置 2–5 分钟时间盒观察 `-symbols=500` 的完成进度、RSS 和 GC。不要首次运行就无时间限制等待全量完成，也不要在基线与候选版本之间并行压测；两者会争用 CPU 和内存并使墙钟结果失真。
...
正常收到 SIGINT/SIGTERM 时，Collector 会先停止 WebSocket 和轮询 producer，再让 worker 最多用 `10s` 排空已经入队的 K 线和爆仓 critical event。超时日志会记录 pending event 和队列长度，剩余 K 线在下次启动时由 REST backfill 补偿。进程崩溃、SIGKILL 或机器断电无法执行该排空流程。
...
- market snapshot bus 的 top500 长时间吞吐、积压和端到端延迟仍需要压测确认；本地合成冷启动已经观察到约 3GB RSS 和明显非线性扩展，当前结果不能作为生产 SLA。
- 服务不提供 HTTP API。
