...
Redis 只承担缓存和当前状态，不承担队列。`market-data` 内部自产自销队列使用代码级约定，不暴露 stream/subject/durable 命名配置；配置只保留 NATS URL、ack wait、最大投递次数、batch 和 worker 开关等运行参数。
...
- 闭合指标和窗口快照写 Redis 时使用一次合并 pipeline，减少 Redis 往返。
- 连续 K 线推进时缓存最近 20 个指标 snapshot，用于窗口分析，避免每次都从零重建窗口 snapshot 序列。
- worker 数按 CPU 自适应并设置上限，避免固定 8 worker 在多 symbol/multi interval 场景下过早成为瓶颈。

最近一次本地指标压测中，`symbols=10`、`tasks=280`、`lookback=100`、模拟 Redis 写延迟 `1ms` 时，单轮从约 `41s` 降到约 `4.9s`。`symbols=100`、`tasks=2800` 的冷启动单轮仍约 `113s`，说明 top500 冷启动仍需要后续异步化、分片或增量预热。
...
最新底层指标快照存储在 Redis。指标不再写入 ClickHouse；需要历史指标时，从 ClickHouse K 线按需重算。
...
Python 策略框架优先从 Redis 读取 Go 已聚合的窗口特征，不再要求实时路径每次从 ClickHouse 拉取几十到几百根 K 线并本地计算指标。
...
该接口面向后续历史回填、回放验证和回测服务。当前实时策略路径仍优先消费 Redis 特征 hash，不依赖每次从 ClickHouse 拉取历史。
...
- NATS 地址、ClickHouse pending 队列和 backfill 异步任务队列。
- 交易对。
- 日志服务名、级别、格式、输出和文件轮转。
