...
ClickHouse 访问实现位于 `pkg/clickhousemarket`。`market-data/internal/store` 只保留服务级入口和 Redis 重试队列，实际 ClickHouse 表初始化、批量写入、K 线历史读取和指标历史读取都由公共包承担。
...
backend/go-service/pkg/
  ...
  clickhousemarket/          # ClickHouse K 线和指标历史读写
...
最新底层指标快照存储在 Redis；启用 ClickHouse 后，每个快照也会作为历史序列数据保留。
...
Python 策略框架优先从 Redis 读取 Go 已聚合的窗口特征，不再要求实时路径每次从 ClickHouse 拉取几十到几百根指标历史。

旧路径仍保留：当特征 hash 不存在时，Python reader 可以读取 Redis 最新指标、最近 K 线和 ClickHouse 指标历史，做本地窗口分析。该路径主要用于兼容和过渡。
...
- `RangeIndicators`：按同样维度读取底层指标快照历史。

这两个接口面向后续历史回填、回放验证和回测服务。当前实时策略路径仍优先消费 Redis 特征 hash，不依赖每次从 ClickHouse 拉取历史。
...
窗口聚合字段会被 Python 解码为 `IndicatorWindowAnalysis`：
