...
早期本地基线约 `84ms/op`、`6.7MB/op`、`4830 allocs/op`。在固定 warmup、窗口/指标拆分、部分指标流式化和重复解析削减后，本地 benchmark 波动较大，只作为趋势参考；`CalculateWindow` 的完整特征计算仍是 realtime CPU 的主要优化方向。
...
```sh
cd backend/go-service/market-data
go run ./cmd/market-data-indicator-loadtest -symbols=500 -lookback=300 -warmup=250 -window-lookback=50 -snapshot-cache-limit=50 -runs=2 -advance-each-run
```

指标负载测试会使用服务当前周期集合和假的 K 线/store 数据模拟四个交易所。它会输出 recent 指标缓存读写、命中/未命中、`calculate_window_calls` 和模拟 Redis 写入次数，不会写入真实存储。带 `-advance-each-run` 时，第二轮开始每个任务只追加一根新收盘 K 线，可用于观察稳态是否只补一根底层指标。
