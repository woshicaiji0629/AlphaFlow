...
- `backtest-engine/cmd` 已收敛为两个长期入口：`backtest-engine` 提供 `run`、`dataset-check`，`market-research` 提供 `swing`、`analysis`、`forward-label`、`structure-regime`、`supertrend-signal`。旧的独立研究 `main` 已迁入 `internal/research` 并删除旧目录。
- `v7-breakout-admission-research-v1` 已因确认过晚、大波段早期覆盖不足而归档为失败实验；实现、专用测试、子命令及独占 Store API 已删除，独立 `market_swings` 数据和波段研究继续保留。
- 已归档 2025-08 至 2025-10 的 2,381 个独立市场波段、V4/V5/V6 命中漏斗和准入实验。直接解除状态锁、方向分数穿越、固定 5/15 分钟加速度及 V7 收盘突破均列为禁止原样重试；成交量/TR 冲击只保留为尚未完成去重单持仓收益验证的候选。
...
- 已新增统一入口下的只读 `market-research forward-label` 子命令和 `forward-market-label.v1` 连续指标协议。协议对每根有效 3m K 线分别观察未来 20/40/80 根闭合 K 线，记录方向收益、半程/后半程收益、最大上下波动、路径效率、已实现波动率、收盘路径回撤/恢复、主导波动位置、回吐、方向优势和阶段扩张；结果侧指标不得进入在线策略输入。
- 已固定首轮标签阈值训练区间为 2024-08-01 至 2024-11-01，冻结验证区间为 2024-11-01 至 2025-01-01。训练报告覆盖 44,160 个观察点，20/40/80 三个窗口均为 44,160 个有效样本、0 个无效样本，并输出总体及逐月 nearest-rank P10 至 P90；冻结验证区间尚未运行或查看。
- Supertrend研究回放已支持标准、自适应和AI三版本的同口径单持仓比较，并记录初始止损、保本、跟踪止损、交易成本、MFE/MAE、利润回吐和多周期入场快照。2025-08基线中标准/自适应均明显亏损，AI仅15笔小幅盈利，尚未形成可上线结论。
- 已完成并删除同趋势结构重入、独立BOS、跨周期BOS回踩、放宽全仓跟踪退出和AI快速因子等失败实验实现；失败原因和关键结果保留在策略文档，不保留临时日志、二进制和配置文件。
...
- 当前AI `1-5`在2025-08只有15笔实际交易。直接提高指标灵敏度或增加结构延续入口均已验证为负期望；少单问题尚未解决，不能通过放宽过滤器强行提高成交数。
- 六类离线行情标签 `trend_up`、`trend_down`、`high_volatility_range`、`low_volatility_consolidation`、`trend_start`、`trend_exhaustion` 已确定互斥语义和判定优先级，但具体数值阈值尚未评审和冻结；训练分布不能直接解释为标签结论或策略收益。
...
1. 只使用 2024-08 至 2024-10 的训练分布提出 20/40/80 三组首版互斥标签阈值，并检查训练期总体及逐月标签占比；不得读取 2024-11 至 2024-12 的冻结验证区间。
2. 阈值评审通过后，将配置、判定优先级、训练区间和分位依据一次性冻结，再运行 2024-11 至 2024-12 验证；验证期不得重新计算分位数或调整阈值。
3. 标签通过冻结验证后，再与全部 AI `1-5` flip（包括市场状态拦截信号）联表，按预先声明的市场状态、方向、波动、趋势强度、均线距离、多周期一致性和 session 分组统计扣费后条件优势。
4. 先补齐指标快照中的 `high` / `low` 并验证 `wick_reclaim` 语义，再将 flip 与 reclaim 分开生成研究数据，不能混成单一信号总体。
5. 统一研究查询口径：指标发现使用“每个信号 × 每种止损一行”的最高止盈潜力标签；固定 TP/SL 表只用于验证可执行退出，不再混用累计命中和互斥最高档分组。
6. 停止沿用未通过 OOS 的候选；新条件必须预先声明并通过逐月及冻结窗口验证。
7. 固定入口与过滤器后，再独立比较现有自适应退出和更简单的止损/止盈方案，避免入口与退出同时变化。
8. 只有 3m 新基线成立后，才接入 1m 精确执行；1m 只在 3m 已产生进出场意图时读取，不参与策略方向判断。
9. 做一次 top500 长时间全链路压测，观察 Redis、NATS market snapshot、NATS strategy decision、ClickHouse、market-data 和策略链路积压；期间用 `make queue-status` 和 `make market-health` 辅助判断队列和可决策状态。
10. 补回测图表报告、结果查询入口和参数化批量运行。
11. 实现 position-engine 的 notify handler。
12. 增加交易所 symbol capability 自动同步和缓存。
13. 明确过期策略反向退出但无 exit rule 时的 action 协议。
14. 使用真实 demo/testnet 凭证完成 execution-engine 端到端验收，再使用最小订单完成 live 安全验收。
...
最近几轮更新包含 NATS JetStream 队列替换、指标流式计算优化、market snapshot bus、strategy-engine 内存市场态、在线/回测统一策略上下文、多策略错误隔离、回测数据完整性检查、多周期流式回放、交易级诊断、独立信号研究和离线行情标签分布。Supertrend flip 的首个指标组合未通过旧 OOS；当前新闭环已完成前向连续指标和未污染训练窗口分布，下一步先评审并冻结六类标签阈值，不读取新冻结验证区间。
