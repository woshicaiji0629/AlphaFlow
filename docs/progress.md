...
- 已新增独立只读 `forward-market-label-research` 命令和 `forward-market-label.v1` 连续指标协议。协议对每根有效 3m K 线分别观察未来 20/40/80 根闭合 K 线，记录方向收益、半程/后半程收益、最大上下波动、路径效率、已实现波动率、收盘路径回撤/恢复、主导波动位置、回吐、方向优势和阶段扩张；结果侧指标不得进入在线策略输入。
