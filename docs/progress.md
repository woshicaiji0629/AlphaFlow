...
1. 对全部AI `1-5` flip（包括市场状态拦截信号）生成统一的20/40/80根前向收益、MFE、MAE和first-passage标签，先判断是否存在可解释的被拦截正期望子集。
2. 先补齐指标快照中的 `high` / `low` 并验证 `wick_reclaim` 语义，再将flip与reclaim分开生成研究数据，不能混成单一信号总体。
3. 统一研究查询口径：指标发现使用“每个信号 × 每种止损一行”的最高止盈潜力标签；固定TP/SL表只用于验证可执行退出，不再混用累计命中和互斥最高档分组。
4. 停止沿用未通过OOS的候选；下一轮重新划分训练与冻结验证区间，并限制预先声明的指标集合和组合数量。
5. 固定入口与过滤器后，再独立比较现有自适应退出和更简单的止损/止盈方案，避免入口与退出同时变化。
6. 只有 3m 新基线成立后，才接入 1m 精确执行；1m 只在 3m 已产生进出场意图时读取，不参与策略方向判断。
7. 做一次 top500 长时间全链路压测，观察 Redis、NATS market snapshot、NATS strategy decision、ClickHouse、market-data 和策略链路积压；期间用 `make queue-status` 和 `make market-health` 辅助判断队列和可决策状态。
8. 补回测图表报告、结果查询入口和参数化批量运行。
