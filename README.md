...
- `forward-market-label-research`：只读回放历史 K 线，生成未来 20/40/80 根 K 线的方向收益、波动、路径效率、回撤、回吐和阶段结构分布，用于离线行情标签研究。
...
│       │   ├── cmd/
│       │   │   ├── backtest-engine/        # 正式历史回测入口
│       │   │   ├── backtest-dataset-check/ # 回测前数据完整性检查入口
│       │   │   ├── supertrend-signal-research/ # Supertrend 信号、路径和场景研究入口
│       │   │   └── forward-market-label-research/ # 离线未来行情指标分布入口
│       │   └── internal/                   # 配置、历史读取、流式模拟和报告计算
