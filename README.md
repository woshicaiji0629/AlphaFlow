...
- Go `strategy-engine` 当前已实现内部 runner 骨架，用于编排策略、仓位计划、订单意图、paper 成交和策略事件。
...
- Go `strategy-engine/internal/runner`：编排策略结果、仓位计划、订单意图、paper 成交、事件持久化和 `bt` / `paper` 本地仓位更新。
...
- `strategy-engine` 服务入口、配置加载和 Redis snapshot reader。
- 回测 CLI / 批处理入口。
...
AlphaFlow/
  backend/
    go-service/
      ...
      strategy-engine/             # Go 在线策略引擎；当前实现 internal/runner
      pkg/
