...
- Go `control-api` 提供 Gin 控制面 API，包括认证、个人 Dashboard、策略目录、管理员策略版本管理和审计。
- `frontend` 是 React 19 + TypeScript + Vite 量化控制台。
...
AlphaFlow/
  backend/
    go-service/
      ...
      control-api/                 # Gin 控制面 API、PostgreSQL migration 和 Controller
      pkg/
        configutil/                # 公共配置解码工具
...
- [docs/control-api.md](docs/control-api.md) 说明控制面 API、权限、策略版本和前端边界。
