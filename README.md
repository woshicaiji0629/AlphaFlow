...
- Go `strategy-engine` 当前已实现在线策略服务，用于读取 Redis 特征快照、运行 Supertrend 策略，并将策略决策发布到 Redis Stream。
- Go `backtest-engine` 当前已实现独立回测入口和配置骨架，后续用于历史 K 线读取、滚动快照、模拟成交和报告输出。
- Go `position-engine` 当前已实现独立仓位/执行路由服务，可长驻消费 Redis Stream 策略决策，并先接入 paper 路由处理器。
...
- Go `strategy-engine` 在线服务：配置加载、Redis snapshot reader、Supertrend 策略注册、runner 编排和策略决策发布。
- Go `backtest-engine` 回测骨架：独立入口、独立配置、历史读取/模拟/报告包边界。
- Go `position-engine` 仓位路由服务：独立入口、独立配置、Redis Stream 长驻消费、策略信号 sink 路由边界和 paper route 处理。
- Go 策略决策总线：`pkg/strategybus`，定义 `strategy.Decision` 的 Redis Stream 输入协议、`trace_id`、`signal_id` 和 result-level signal id。
...
AlphaFlow/
  backend/
    go-service/
      ...
      position-engine/             # Go 仓位/执行路由服务；当前实现入口、配置、消费循环和 route 边界
      pkg/
        configutil/                # 公共配置解码工具
...
- `position-engine`：独立仓位/执行路由入口，当前可长驻消费 Redis Stream 策略决策，并把 paper route 交给 `pkg/positionhandler/paper` 处理。
- `pkg/strategybus`：策略决策 Redis Stream 协议，默认 stream 为 `st:decision:stream`，payload 字段为 `payload`，envelope 带 `trace_id` / `signal_id`。
...
- 通过 `pkg/strategybus` 将策略决策发布到 Redis Stream。

当前 `position-engine` 已支持：

- 长驻消费 Redis Stream 策略决策和 pending 消息。
- result-level 幂等处理。
...
`position-engine` 与在线/回测数据源独立。在线和回测只负责产出策略决策；决策通过 `pkg/strategybus` 定义的 Redis Stream 协议进入 position-engine。当前 paper route 已接入公共 paper handler，并可从 Redis `lp/mp` 价格 key 补最新价格上下文；服务长驻消费新消息和 pending 消息，空批次带 backoff；处理成功后 Ack，过期开仓类信号跳过并 Ack，过期退出类信号会用当前持仓 exit rules 和最新价格做保守重裁决；处理失败的 pending 消息可 reclaim，超过投递上限后进入 dead-letter stream。position-engine 已接入 `pkg/idempotency`，优先按 result-level signal id 做幂等，兼容旧消息的 message id 幂等。backtest、live 和 notify handler 后续补齐。
