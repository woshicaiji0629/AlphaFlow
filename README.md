...
- Go `market-data` 负责交易所行情采集、派生 K 线、指标计算、指标窗口聚合、Redis 实时状态和 ClickHouse 历史写入。
- Python `alphaflow-core` 负责读取 Redis 特征快照、执行可插拔策略、管理策略仓位原型。
- Redis 用于实时缓存和活跃仓位。
- ClickHouse 用于已闭合 K 线和底层指标历史。
...
指标计算只使用已闭合 K 线。底层指标快照按动态 map 存储：
...
在底层指标之上，Go `indicatorwindow` 会生成窗口特征。窗口特征不是新的长期历史源，而是基于底层指标序列聚合出来的二级数据。它可以随口径变化重新计算，策略优先消费这层语义化结果。
...
- 在 Redis 特征 hash 不存在时，兼容旧路径读取最新指标、健康状态、最近 K 线和 ClickHouse 指标历史。
- 构造 `MarketSnapshot`。
- 运行所有已注册策略。
- 维护 Redis 活跃仓位。
- 将已平仓仓位写入 PostgreSQL。
...
ClickHouse 仍保存底层 K 线和指标序列，主要用于研究、回测、重新计算窗口口径和问题追溯。实时策略路径不再要求 Python 每次读取几十到几百根历史指标。
...
如果配置了 `ALPHAFLOW_POSTGRES_DSN`，策略服务启动时会初始化已平仓仓位表。如果配置了 `ALPHAFLOW_CLICKHOUSE_HTTP_URL`，策略服务可在旧兼容路径中读取最近指标历史；当前实时策略路径优先读取 Redis 特征 hash。
