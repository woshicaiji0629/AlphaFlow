...
- Go `strategy-engine` 当前已实现在线策略服务，用于读取 Redis 特征快照、按配置加载策略、运行策略集合，并将策略决策发布到 Redis Stream。
- Go `backtest-engine` 当前已实现独立回测初版链路，包括历史 K 线读取、滚动快照、策略执行、模拟成交、交易明细和摘要持久化。
- Go `position-engine` 当前已实现独立仓位/执行路由服务，可长驻消费 Redis Stream 策略决策、处理 paper 路由，并滚动扫描 paper 持仓的退出规则。
...
- Go `position-engine` 仓位路由服务：独立入口、独立配置、Redis Stream 长驻消费、策略信号 sink 路由边界、paper route 处理和 paper 持仓 scanner。
...
- `position-engine`：独立仓位/执行路由入口，当前可长驻消费 Redis Stream 策略决策，并把 paper route 和 paper 持仓 scanner 交给公共仓位/执行能力处理。
