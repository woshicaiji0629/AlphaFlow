...
- Go `pkg/` 提供可复用基础能力，包括交易所 REST K 线客户端、ClickHouse K 线读写、公共市场模型、market snapshot bus、纯指标计算、指标窗口分析、策略模型、仓位管理和执行协议。
...
- `strategy-engine/internal/reader`：读取 Redis `indwin` / `indrt` 并构造启动恢复用 `strategy.Snapshot`。
