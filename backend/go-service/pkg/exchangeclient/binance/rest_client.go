...
func (c *RESTClient) FetchOpenInterest(
	...
	if err != nil {
		return marketmodel.OpenInterest{}, fmt.Errorf("fetch open interest: %w", err)
	}
	...
	if err := json.Unmarshal(body, &raw); err != nil {
		return marketmodel.OpenInterest{}, fmt.Errorf("decode open interest: %w", err)
	}

	return marketmodel.OpenInterest{
		Exchange:     "binance",
		Market:       "um",
		Symbol:       raw.Symbol,
...
func parseRESTKline(symbol string, interval string, item []any) (marketmodel.Kline, error) {
	if len(item) < 11 {
		return marketmodel.Kline{}, fmt.Errorf("invalid rest kline length: %d", len(item))
	}
	...
	if err != nil {
		return marketmodel.Kline{}, fmt.Errorf("parse close time: %w", err)
	}
	tradeCount, err := numberAsInt64(item[8])
	if err != nil {
		return marketmodel.Kline{}, fmt.Errorf("parse trade count: %w", err)
	}
