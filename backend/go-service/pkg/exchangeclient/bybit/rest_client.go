...
func (c *RESTClient) FetchKlines(
	...
	klines := make([]marketmodel.Kline, 0, len(response.Result.List))
...
func (c *RESTClient) klineFromRaw(symbol string, interval string, raw []string) (marketmodel.Kline, error) {
	if len(raw) < 7 {
		return marketmodel.Kline{}, fmt.Errorf("invalid bybit kline length: %d", len(raw))
	}
