...
func addEnhancedToSet(target *ValueSet, values map[string]string, signals map[string]string, opens []float64, highs []float64, lows []float64, closes []float64, volumes []float64, basic *basicIndicatorState, features *featureContext, preview *aiSourceResult) {
	...
	addHeikinAshiFeaturesToSet(target, values, signals, opens, highs, lows, closes)
	addSupportResistanceToSet(target, values, signals, highs, lows, closes)
	...
	addAlphaTrendToSet(target, values, signals, highs, lows, closes, volumes, 14, 1)
	addPSARFeaturesToSet(target, values, signals, highs, lows, closes)
	addChandelierExitToSet(target, values, signals, highs, lows, closes, 22, 3)
	...
	addDynamicSwingAnchoredVWAPToSet(target, values, signals, highs, lows, closes, volumes)
