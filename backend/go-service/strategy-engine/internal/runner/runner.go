...
func (r *Runner) Handle(ctx context.Context, input strategy.Context) (strategy.Decision, error) {
	...
	for _, result := range decision.Results {
		if err := r.handleResult(ctx, input, result); err != nil {
			return strategy.Decision{}, err
		}
	}
	return decision, nil
...
func (r *Runner) handleResult(ctx context.Context, input strategy.Context, result strategy.Result) error {
	now := r.now()
	currentPosition := input.Positions[result.StrategyName]
	price := currentPrice(input)
	refreshedPosition, err := r.refreshLocalPosition(ctx, input.Target, result.StrategyName, currentPosition, price)
	if err != nil {
		return err
	}
	currentPosition = refreshedPosition
	if err := r.appendEvent(ctx, signalEvent(input.Target, result, now)); err != nil {
		return err
	}
	plan := r.positionManager.PlanWithPrice(result, currentPosition, price)
	orderPlan, err := r.orderPlanWithQuantity(*plan, price)
	if err != nil {
	...
	if err := r.appendEvent(ctx, orderIntentEvent(input.Target, result, intent, plan, now)); err != nil {
		return err
	}
	if r.broker == nil {
		return nil
	}
	report, err := r.broker.Execute(ctx, intent)
	if err != nil {
		return fmt.Errorf("execute order intent %s: %w", intent.IntentID, err)
	}
	if err := r.appendEvent(ctx, orderFilledEvent(input.Target, result, currentPosition, intent, report, plan, r.feeConfig, r.sizingConfig, now)); err != nil {
		return err
	}
	if err := r.applyLocalFill(ctx, input.Target, result, currentPosition, intent, report, plan); err != nil {
		return fmt.Errorf("apply local fill for strategy %s: %w", result.StrategyName, err)
	}
	return nil
}

func (r *Runner) appendEvent(ctx context.Context, event strategy.StrategyEvent) error {
	if r.eventStore == nil {
		return nil
	}
	if err := r.eventStore.AppendEvent(ctx, event); err != nil {
...
func (r *Runner) orderPlanWithQuantity(plan strategy.OrderPlan, price string) (strategy.OrderPlan, error) {
	switch plan.Action {
	case strategy.PositionActionOpenLong, strategy.PositionActionOpenShort:
		if plan.TargetSize <= 0 {
			return plan, nil
		}
		if r.sizingConfig.MarginQuote <= 0 || r.sizingConfig.Leverage <= 0 {
			return plan, nil
		}
...
func (r *Runner) refreshLocalPosition(
	ctx context.Context,
	...
	refreshed.UpdatedAt = r.now()
	if err := r.positionStore.SavePosition(ctx, refreshed); err != nil {
		return nil, fmt.Errorf("refresh position extremes for strategy %s: %w", strategyName, err)
	}
	return &refreshed, nil
}

func (r *Runner) applyLocalFill(
	ctx context.Context,
	target strategy.Target,
	...
	case execution.OrderActionOpen:
		return r.positionStore.SavePosition(ctx, strategy.Position{
			Scope:        target.Scope,
			RunID:        target.RunID,
			Exchange:     target.Exchange,
			Market:       target.Market,
	...
	case execution.OrderActionClose:
		return r.positionStore.DeletePosition(ctx, positionKey(target, result.StrategyName))
	case execution.OrderActionReduce:
		...
		return r.positionStore.SavePosition(ctx, updated)
	default:
		return nil
	}
}
